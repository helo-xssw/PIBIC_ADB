...
with open(METADATA_FILE, "w", encoding="utf-8") as f:
    ...
    f.write(f"Método: signalstrength\n")
    f.write(f"Intervalo: {INTERVALO}s\n")
    f.write(f"Duração: {DURACAO}s\n")
    f.write(f"Data: {timestamp}\n")

print("\n[INFO] Iniciando coleta SignalStrength...")
        ...
        try:
            ...
            if linhas_filtradas:
                ...
                print(
                    ...
                    f"{momento.split()[1]} | "
                    f"Linhas RF: {len(linhas_filtradas)}"
                )

            else:

                print(
                    f"[AVISO] Nenhum dado RF encontrado às "
                    f"{momento.split()[1]}"
                )
...
print(f"[INFO] Total de linhas RF salvas: {linhas_salvas}")
